    ...
    .map(
      (l) => {
        ...
        <tr>
          ...
          <td class="num">${l.gstRate.toFixed(1)}%</td>
...
<body>
  ...
  <div class="meta">
    ...
    <div><div class="lbl">Payment Status</div><div>${esc(paymentInfo.statusText)}</div></div>
  </div>
  ...
  <div class="totals">
    ...
    <div class="totbox">
      ...
      <div class="row"><span>Amount Paid</span><span>${money(paymentInfo.amountPaid)}</span></div>
      <div class="row"><span>Amount To Be Paid</span><span>${money(paymentInfo.balanceAmount)}</span></div>
      <div class="row"><span>Payment Status</span><span><strong>${esc(paymentInfo.statusText)}</strong></span></div>
      <div class="row"><span>Total GST</span><span>${money(order.gstTotal)}</span></div>
...
export function invoiceWhatsappText(order: SalesOrder, s: AppSettings): string {
  ...
  const lines = order.lines
    .map((l) => `• ${l.name} ×${l.qty} — ${money(lineNet(l) + lineGst(l))}`)
    .join("\n");
  return [
    `*${s.companyName}*`,
    `Invoice ${inv}`,
    ...
    `Amount Paid: ${money(paymentInfo.amountPaid)}`,
    `Amount To Be Paid: ${money(paymentInfo.balanceAmount)}`,
    `GST Total: ${money(order.gstTotal)}`,
    `*Payment Status: ${paymentInfo.statusText}*`,
    "",
