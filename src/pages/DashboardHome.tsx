...
export default function DashboardHome() {
  ...
  const salesOrders = useMemo(() => {
    ...
    // 1. Process inventory salesOrders with normalized lines
    (invSalesOrders || []).forEach((o: any) => {
      ...
      const lines = rawLines.map((item: any) => {
        ...
        const cost = Number(item.cost ?? p?.costPrice ?? 0);
        const price = Number(item.price ?? item.unitPrice ?? p?.sellingPrice ?? 0);
        const qty = Number(item.quantity ?? item.qty ?? 1) || 1;
        const discount = Number(item.discount ?? 0);
        return {
          productId,
          name: item.name || item.title || item.productName || p?.name || "",
          sku: item.sku || item.productId || p?.sku || "",
          qty,
          price,
          cost,
      ...
      const calculatedProfit = lines.reduce((s: number, l: any) => s + (l.price - l.cost) * l.qty - l.discount, 0);

      map.set(o.id, {
        ...o,
        lines,
        total: Number(o.total ?? o.amount ?? o.totalAmount ?? 0),
        profit: Number(o.profit ?? calculatedProfit),
  ...
  }, [invSalesOrders, adminOrders, products]);
